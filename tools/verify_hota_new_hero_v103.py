#!/usr/bin/env python3
"""Verify HOTA_NEW_HERO_V1.03 release invariants."""
...
def main() -> int:
    ...
    if args.source_zip and sha256_file(args.source_zip.resolve()) != SOURCE_ZIP_SHA256:
        raise RuntimeError("Source V1.02 hash mismatch")
    ...
    for name in EXE_NAMES:
        ...
        print(f"{name}: F6 formula + Adela + Elf invariants=PASS")

    hota_path = release_root / "HotA.dat"
    ...
    if hota_data[skill_offset : skill_offset + len(ASTRA_SKILLS_AFTER)] != ASTRA_SKILLS_AFTER:
        raise RuntimeError("HotA.dat: Astra Wisdom/Water skill block mismatch")
    astra_manifest = manifest["astra_starting_skills"]
    if sha256_file(hota_path) != astra_manifest["output_sha256"]:
        raise RuntimeError("HotA.dat: manifest hash mismatch")
    if astra_manifest["output_skill_block_hex"] != ASTRA_SKILLS_AFTER.hex(" "):
        raise RuntimeError("HotA.dat: manifest skill block mismatch")
    print("HotA.dat: Astra Basic Wisdom + Basic Water Magic=PASS")
    ...
    for (power, level, tier, water), expected in expected_samples.items():
        actual = total_cure(power, level, tier, water)
        if actual != expected:
            raise RuntimeError(
                f"Formula sample mismatch P={power}, L={level}, n={tier}, w={water}: "
                f"{actual} != {expected}"
            )
    print("F6 Direct formula sample matrix=PASS")
