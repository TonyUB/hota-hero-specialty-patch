#!/usr/bin/env python3
"""Verify HOTA_NEW_HERO_V1.01 release invariants."""
...
def main() -> int:
    ...
    if args.source_zip and sha256_file(args.source_zip.resolve()) != SOURCE_ZIP_SHA256:
        raise RuntimeError("Source V1 hash mismatch")
    ...
    source_text = SOURCE_CURE_TEXT.encode("gb18030")
    for relative in LANGUAGE_ARCHIVES:
        member = hero_spec_member(release_root / relative)
        if member.count(release_text) != 1 or source_text in member:
    ...
    for (power, level, tier), expected in expected_samples.items():
        actual = tuple(total_cure(power, level, tier, mastery) for mastery in (1, 2, 3))
        if actual != expected:
            raise RuntimeError(
                f"Formula sample mismatch P={power}, L={level}, n={tier}: {actual} != {expected}"
            )
    print("Formula sample matrix=PASS")
