#!/usr/bin/env python3
"""Verify the V1.04 UI/log candidate and F6 specialty-panel fixes."""
...
from build_hota_new_hero_v104_ui_test3 import (
    APPEND_HELPER_VA,
    BUILD_NAME,
    CORPSE_CALC_CALL_VA,
...
def main() -> int:
    ...
    ui_helper, _ = build_hota_ui_helper()
    ...
    old_description = ASTRA_HDAT_DESCRIPTION_OLD.encode("gb18030")
    new_description = ASTRA_HDAT_DESCRIPTION_NEW.encode("gb18030")
    if old_description in hota_data or hota_data.count(new_description) != 1:
        raise RuntimeError("HotA.dat Astra Cure description mismatch")
    if sha256_file(hota_path) != manifest["hota_dat_description"]["output_sha256"]:
        raise RuntimeError("HotA.dat manifest hash mismatch")
    ...
    print("HotA.dat Astra Cure description + Wisdom/Water skills=PASS")

    hota_dll_source = args.hota_dll.resolve()
    if sha256_file(hota_dll_source) != HOTA_DLL_SOURCE_SHA256:
        raise RuntimeError("Clean HotA.dll hash mismatch")
    hota_dll_path = candidate_root / HOTA_DLL_NAME
    hota_dll = hota_dll_path.read_bytes()
    hota_pe = pefile.PE(data=hota_dll, fast_load=False)
    helper_target = HOTA_UI_HELPER_VA.to_bytes(4, "little")
    for va in (HOTA_UI_EFFECT_TARGET_VA, HOTA_UI_PERCENT_TARGET_VA):
        offset = va_to_offset(hota_pe, va)
        if hota_dll[offset : offset + 4] != helper_target:
            raise RuntimeError(f"HotA.dll UI dispatcher mismatch at {va:#x}")
    hota_manifest = manifest["hota_dll"]
    if sha256_file(hota_dll_path) != hota_manifest["output_sha256"]:
        raise RuntimeError("HotA.dll manifest hash mismatch")
    rollback = bytearray(hota_dll)
    for region in reversed(hota_manifest["logical_patch_regions"]):
        start = int(region["file_offset"])
        rollback[start : start + int(region["length"])] = bytes.fromhex(region["rollback_hex"])
    if bytes(rollback) != hota_dll_source.read_bytes():
        raise RuntimeError("HotA.dll rollback mismatch")
    print("HotA.dll F6 specialty-panel dispatch + rollback=PASS")
