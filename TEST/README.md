...
| `Patch_v2.6_VISUAL_LOGDIAG01` | 一次群体治愈记录包装器、复活计数以及轮转前/后/刷新后的向量快照 | `38e6c25410a616ae704624c5469baae84c38d1d3ef7b2cf04c6868edd0265214` |
...
TEST12 实机仍显示群体治愈记录在最后，因此停止继续生成猜测式顺序补丁。`Patch_v2.6_VISUAL_LOGDIAG01` 保持 TEST12 行为，只增加固定 24 字节二进制记录：事件 1–3 证明群体包装器、尸体辅助函数和每次治愈复活计数是否执行；事件 4–6 分别保存原生治愈日志追加后、指针轮转后和原生刷新后的向量状态。一次高级水系群体治愈后上传 `hota_cure_logdiag01.bin` 即可定位第一处失效点。
