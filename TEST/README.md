...
| `Patch_v2.6_VISUAL_TEST13` | 已通过并提升为正式 v2.6：群体治愈先显示施法记录，再显示各队复活记录，功能与演出均正常 | `2a4a8c6cd3177452680c5542ea9fa0ee7a8a3afe881e4f51d2322bc7410896cb` |
...
`Patch_v2.6_VISUAL_TEST13` 据此改为真正调整追加时序：仅在群体治愈作用域内暂存每队兵种与复活数量，跳过当时的原生复活日志调用；原版治愈行追加后，复用原版格式串和原生 `0x004729D0` 依次补写复活行。单体治愈和普通转世重生直接使用原调用，不进入暂存路径。标准版/HD 版双独立构建、`RET 0x0C` 栈清理、原生格式化与日志调用顺序、完整回滚和 ZIP CRC 均通过。实机截图确认群体治愈记录已位于全部复活记录之前，且功能无回归；该执行文件已逐字节提升为正式 `Patch_v2.6`。
