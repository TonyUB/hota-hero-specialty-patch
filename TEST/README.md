...
| `Patch_v2.6_VISUAL_TEST11` | 直接包装实机可观察的原生效果检查和治愈日志调用，由原日志函数返回后轮转并刷新 | `c646a47879db8d4ef7a62e6e999041c554db10553a178c7cf3b6d6185940cfaa` |
...
TEST10 实机仍显示群体治愈记录在最后。`Patch_v2.6_VISUAL_TEST11` 不再使用独立前后跳板：`0x005A1B97` 的原生效果检查调用负责记录起点，`0x005A1BF6` 的原生治愈日志调用直接返回到轮转延续代码；只要屏幕出现“施放治愈”，该延续路径就已经执行。日志容器已静态确认是 4 字节指针数组，延续代码轮转后调用原生日志刷新。
