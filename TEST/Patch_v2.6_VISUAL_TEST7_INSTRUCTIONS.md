...
状态：**已撤回。单体日志顺序正确，但群体在 `0x005A1B78` 崩溃；请改用 TEST8。**

TEST7 对整段群体循环进行了重编译。最新实机报告显示，群体治愈在目标索引计算阶段 `0x005A1B78` 读取无效地址；此时尚未进入尸体扫描、施法记录或日志轮转。该结果证明 HotA/HD Hook 依赖 TEST4 的原始指令地址布局。不要继续安装或测试本包。
