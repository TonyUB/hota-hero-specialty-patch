# Patch_v2.5_STAGE3_TEST 测试说明

状态：**已撤回，启动必然失败，禁止安装。以下内容是当时的原始说明，仅供取证。**

故障原因：本包覆盖了 `MainProc\0` 的字符串终止符。请改用 `Patch_v2.5_STAGE3_TEST2.zip`。
...
1. 备份当前游戏目录中的同名文件。
2. 将 `Patch_v2.5_STAGE3_TEST.zip` 解压到 HotA 1.8.0 游戏根目录并覆盖。
