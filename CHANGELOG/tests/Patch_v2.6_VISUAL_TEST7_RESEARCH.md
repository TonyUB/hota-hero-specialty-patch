...
状态：**已撤回。单体路径通过，但群体在重编译循环内 `0x005A1B78` 崩溃。**

## TEST7 实机结论

- 单体治愈日志顺序正确：先显示施放治愈，再显示起死回生。
- 群体治愈异常 EIP 为 `0x005A1B78`，读取无效地址 `0xF6715754`。
- 崩溃发生在 TEST7 重编译后的目标索引计算中，早于尸体扫描、原版 `0x005A8C60` 施法记录和日志轮转。
- 因此 TEST7 的日志轮转算法尚未得到群体路径执行验证；根因是整段循环重排破坏了 HotA/HD 的地址敏感 Hook 条件。TEST8 改为保留 TEST4 原地址，仅使用固定长度跳板。
